...
bool Check4Keystroke(void)
{
  ...
  if ( IsNewKeyReady() ) // new key waiting?
  {
    ...
    if(Key == 'a')
    {
        ThisEvent.EventType = ES_ButtonDown;
        PostYeezy(ThisEvent); 
    }
    if(Key == 'b')
    {
        ThisEvent.EventType = ES_HandsDetected;
        // Post
    }
    if(Key == 'c')
    {
        ThisEvent.EventType = ES_FaderChange;
        ThisEvent.EventParam = 1000;
        PostGlasses(ThisEvent);
    }
    if(Key == 'd')
    {
        ThisEvent.EventType = ES_NewAudio;
        // Post
        PostAudio(ThisEvent);
    }
    if(Key == 'e')
    {
        ThisEvent.EventType = ES_Celebration;
        PostHypeLights(ThisEvent);
    }
    if(Key == 'f')
    {
        ThisEvent.EventType = ES_CelebrationComplete;
        PostHypeLights(ThisEvent);
        PostAudio(ThisEvent);
    }
    if(Key == 'g')
    {
        ThisEvent.EventType = ES_HypeLights;
        PostHypeLights(ThisEvent);
    }
    if(Key == 'h')
    {
    ...
    if(Key == 'i')
    {
        ThisEvent.EventType = ES_MoveToRecord;
        // Post
    }
    if(Key == 'j')
    {    
